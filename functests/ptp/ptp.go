...
var _ = Describe("[ptp]", func() {
	...
	Context("PTP configuration verifications", func() {
		...
		It("Should check whether PTP operator appropriate resource exists", func() {
			...
			rl, err := testclient.Client.ServerPreferredResources()
		...
		It("Should check that all nodes are running at least one replica of linuxptp-daemon", func() {
			...
			nodes, err := testclient.Client.Nodes().List(metav1.ListOptions{})
			...
			ds, err := testclient.Client.DaemonSets(PtpLinuxDaemonNamespace).List(metav1.ListOptions{})
			Expect(err).ToNot(HaveOccurred())
		...
		It("Should check that operator is deployed", func() {
			...
			dep, err := testclient.Client.Deployments(PtpLinuxDaemonNamespace).Get(PtpOperatorDeploymentName, metav1.GetOptions{})
			Expect(err).ToNot(HaveOccurred())
	...
	Describe("PTP e2e tests", func() {
		...
		execute.BeforeAll(func() {
			ptpconfigList, err := testclient.Client.PtpConfigs(PtpLinuxDaemonNamespace).List(metav1.ListOptions{})
			Expect(err).ToNot(HaveOccurred())

			for _, ptpConfig := range ptpconfigList.Items {
				err = testclient.Client.PtpConfigs(PtpLinuxDaemonNamespace).Delete(ptpConfig.Name, &metav1.DeleteOptions{})
				Expect(err).ToNot(HaveOccurred())
			}

			nodeList, err := testclient.Client.Nodes().List(metav1.ListOptions{LabelSelector: fmt.Sprintf("%s=", PtpGrandmasterNodeLabel)})
			Expect(err).ToNot(HaveOccurred())
			for _, node := range nodeList.Items {
				delete(node.Labels, PtpGrandmasterNodeLabel)
				_, err = testclient.Client.Nodes().Update(&node)
				Expect(err).ToNot(HaveOccurred())
			}

			nodeList, err = testclient.Client.Nodes().List(metav1.ListOptions{LabelSelector: fmt.Sprintf("%s=", PtpSlaveNodeLabel)})
			Expect(err).ToNot(HaveOccurred())
			for _, node := range nodeList.Items {
			...
			ptpNodes, err := nodes.GetNodeTopology(PtpLinuxDaemonNamespace)
			Expect(err).ToNot(HaveOccurred())
			...
			ptpPods, err := testclient.Client.Pods(PtpLinuxDaemonNamespace).List(metav1.ListOptions{LabelSelector: "app=linuxptp-daemon"})
			Expect(err).ToNot(HaveOccurred())
			for _, pod := range ptpPods.Items {
				err = testclient.Client.Pods(PtpLinuxDaemonNamespace).Delete(pod.Name, &metav1.DeleteOptions{GracePeriodSeconds: pointer.Int64Ptr(0)})
				Expect(err).ToNot(HaveOccurred())
			}

			daemonset, err := testclient.Client.DaemonSets(PtpLinuxDaemonNamespace).Get(PtpDaemonsetName, metav1.GetOptions{})
			Expect(err).ToNot(HaveOccurred())
				...
				for _, pod := range ptpRunningPods {
					if podRole(pod, PtpSlaveNodeLabel) {
						Eventually(func() string {
							buf, _ := pods.ExecCommand(testclient.Client, pod, []string{"curl", "127.0.0.1:9091/metrics"})
							timeDiff = buf.String()
							return timeDiff
	...
	Describe("Test NodeName Selector And Policy Priority", func() {
		...
		BeforeEach(func() {
			...
			response, err := testclient.Client.CoreV1Interface.RESTClient().Get().AbsPath(ConfigPtpAPIPath + "/slave").DoRaw()
			Expect(err).NotTo(HaveOccurred())
			...
			status := testclient.Client.CoreV1Interface.RESTClient().Post().AbsPath(ConfigPtpAPIPath).
		...
		AfterEach(func() {
			status := testclient.Client.CoreV1Interface.RESTClient().Delete().AbsPath(ConfigPtpAPIPath + "/test").Do()
...
func podRole(runningPod v1core.Pod, role string) bool {
	nodeList, err := testclient.Client.Nodes().List(metav1.ListOptions{
		LabelSelector: role,
	})
	Expect(err).NotTo(HaveOccurred())
...
func ptpDiscoveredInterfaceList(path string) []string {
	...
	fg, err := testclient.Client.CoreV1Interface.RESTClient().Get().AbsPath(path).DoRaw()
	Expect(err).ToNot(HaveOccurred())
...
func getPtpPodOnNode(nodeName string) (v1core.Pod, error) {
	runningPod, err := testclient.Client.Pods(PtpLinuxDaemonNamespace).List(metav1.ListOptions{LabelSelector: "app=linuxptp-daemon"})
...
func getMasterSlaveAttachedInterfaces(pod v1core.Pod) []string {
	...
	Eventually(func() error {
		stdout, err := pods.ExecCommand(testclient.Client, pod, []string{"ls", "/sys/class/net/"})
		if err != nil {
			return err
		}
...
func getPtpMasterSlaveAttachedInterfaces(pod v1core.Pod) []string {
	...
	for _, interf := range intList {
		...
		// Get readlink status
		Eventually(func() error {
			stdout, err = pods.ExecCommand(testclient.Client, pod, []string{"readlink", "-f", fmt.Sprintf("/sys/class/net/%s", interf)})
			if err != nil {
				return err
			}
		...
		Eventually(func() error {
			...
			stdout, err = pods.ExecCommand(testclient.Client, pod, []string{"ls", fmt.Sprintf("/sys/bus/pci/devices/%s/physfn", PCIAddr)})
			if err != nil {
		...
		Eventually(func() error {
			stdout, err = pods.ExecCommand(testclient.Client, pod, []string{"ethtool", "-T", interf})
			if stdout.String() == "" {
				return fmt.Errorf("empty response from pod retrying")
			}
...
func replaceTestPod(pod v1core.Pod, timeout time.Duration) (v1core.Pod, error) {
	...
	err := testclient.Client.Pods(PtpLinuxDaemonNamespace).Delete(pod.Name, &metav1.DeleteOptions{
...
func createConfig(profileName, ifaceName, ptp4lOpts, phc2sysOpts, nodeLabel string, priority *int64) error {
	...
	_, err := testclient.Client.PtpConfigs(PtpLinuxDaemonNamespace).Create(&policy)
	return err
}
