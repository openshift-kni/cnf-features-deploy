	...
	Context("tuningcni over bond", func() {
		It("pods with sysctls over bond should be able to ping each other",
			func() {
				...
				bondWithTuningNad1, err := networks.NewNetworkAttachmentDefinitionBuilder(TestNamespace, bondNadName1).WithBond("net3", "net1", "net2").WithStaticIpam(ip1).WithTuning(sysctls).Build()
				Expect(err).ToNot(HaveOccurred())
				err = client.Client.Create(context.Background(), bondWithTuningNad1)
				Expect(err).ToNot(HaveOccurred())

				bondWithTuningNad2, err := networks.NewNetworkAttachmentDefinitionBuilder(TestNamespace, bondNadName2).WithBond("net3", "net1", "net2").WithStaticIpam("10.10.0.23").WithTuning(sysctls).Build()
				Expect(err).ToNot(HaveOccurred())
