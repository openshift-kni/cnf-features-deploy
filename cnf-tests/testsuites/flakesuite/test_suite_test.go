...
import (
	...
	_ "github.com/openshift-kni/cnf-features-deploy/cnf-tests/testsuites/flakesuite/empty_string"
...
func TestTest(t *testing.T) {
	...
	if *junitPath != "" {
		junitFile := path.Join(*junitPath, "flakes_junit.xml")
		rr = append(rr, reporters.NewJUnitReporter(junitFile))
	}
	if *reportPath != "" {
		reportFile := path.Join(*reportPath, "flakes_failure_report.log")
	...
	RunSpecsWithDefaultAndCustomReporters(t, "CNF Features flakes test", rr)
}
