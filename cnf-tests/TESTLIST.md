...
| validation sro should have a ready deployment for the NFD Operator | Check that node feature discovery operator is deploy and running as expected | 
| validation sro should have a ready deployment for the Special Resource Operator | Check that special resource operator is deploy and running as expected | 
| validation sro should have at least one nfd CR apply on the cluster to deploy the operand daemonsets | Check a CR for node feature discovery exist on the cluster to deploy the operands | 
