...
| [performance] CPU Management Hyper-thread aware scheduling for guaranteed pods Verify Hyper-Thread aware scheduling for guaranteed pods  HT aware scheduling on SNO cluster | Verifies Hyper-Thread aware scheduling for guaranteed pods on SNO cluster | 
| [performance] CPU Management Hyper-thread aware scheduling for guaranteed pods Verify Hyper-Thread aware scheduling for guaranteed pods  HT aware scheduling on SNO cluster and Workload Partitioning enabled | Verifies Hyper-Thread aware scheduling for guaranteed pods on SNO cluster and Workload Partitioning enabled | 
| [performance] CPU Management Hyper-thread aware scheduling for guaranteed pods Verify Hyper-Thread aware scheduling for guaranteed pods  Number of CPU requests as multiple of SMT count allowed when HT enabled | Verifies Hyper-Thread aware scheduling for guaranteed pods when number of CPU requests as multiple of SMT count allowed when HT enabled | 
| [performance] CPU Management Hyper-thread aware scheduling for guaranteed pods Verify Hyper-Thread aware scheduling for guaranteed pods  Odd number of CPU requests allowed when HT disabled | Verifies Hyper-Thread aware scheduling for guaranteed pods when odd number of CPU requests allowed when HT disabled | 
...
| [performance] Checking IRQBalance settings Verify irqbalance configuration handling Should not overwrite the banned CPU set on tuned restart | Verify irqbalance configuration handling is not overwritten the banned CPU set on tuned restart | 
