...
| [performance] CPU Management when strict NUMA aligment is requested  should reject pods which request integral CPUs not aligned with machine SMT level | Verifies that once strict NUMA alignment is requested the CPU manager reject pods that requested number of CPUs not aligned with machine SMT level | 
