...
| Test Name | Description |
| -- | ----------- |
| [performance]  Should have kernel param rcutree.kthread | Checks that the kernel params includes RCU options | 
...
| [performance] Checking IRQBalance settings Verify irqbalance configuration handling Should DO overwrite the banned CPU set on CRI-O restart | Test CRI-O banned cpu list is correctly cleaned up | 
| [performance] Checking IRQBalance settings Verify irqbalance configuration handling Should not overwrite the banned CPU set on tuned restart | Test CRI-O banned cpu list is preserved across tuned restarts | 
| [performance] Checking IRQBalance settings Verify irqbalance configuration handling Should store empty cpu mask in the backup file | Test CRI-O banned cpu list is correctly initialized | 
...
| [performance] RPS configuration Should not have RPS configuration set when realtime workload hint is explicitly set | Validates RPS hooks are disabled when configuring for realtime workload | 
