...
| [performance] Tuned CRs generated from profile Tuned profile shouldn't be degraded | Validates that Tuned CRs generated from Tuned profile are not degraded | 
...
| [performance] Using performance profile [test_id: 73107] Should have system services running on the system.slice cgroup | validates that system services are running under system.slice cgroup | 
...
| [ref_id: 40307][pao]Resizing Network Queues Updating performance profile for netqueues  Verify reserved cpus count is applied to all but specific supported networking device using a negative match | Validates that reserved cpus count is applied to all but specific supported networking device using a negative match | 
