...
| Test Name | Description |
| -- | ----------- |
| [performance]  Should have kernel param rcutree.kthread | Verifies that kernel param rcutree.kthread appears under the node and has priority 11 | 
...
| [performance] CPU Management Hyper-thread aware scheduling for guaranteed pods Verify Hyper-Thread aware scheduling for guaranteed pods  HT aware scheduling on SNO cluster | Verifies HT aware scheduling for guaranteed pods on SNO cluster | 
| [performance] CPU Management Hyper-thread aware scheduling for guaranteed pods Verify Hyper-Thread aware scheduling for guaranteed pods  HT aware scheduling on SNO cluster and Workload Partitioning enabled | Verifies HT aware scheduling for guaranteed pods on SNO cluster when Workload Partitioning is enabled | 
| [performance] CPU Management Hyper-thread aware scheduling for guaranteed pods Verify Hyper-Thread aware scheduling for guaranteed pods  Number of CPU requests as multiple of SMT count allowed when HT enabled | Verifies that number of CPU requests as multiple of SMT count is allowed when HT enabled | 
| [performance] CPU Management Hyper-thread aware scheduling for guaranteed pods Verify Hyper-Thread aware scheduling for guaranteed pods  Odd number of CPU requests allowed when HT disabled | Verifies that Odd number of CPU requests allowed when HT disabled | 
...
| [performance] Checking IRQBalance settings Verify irqbalance configuration handling Should DO overwrite the banned CPU set on CRI-O restart | Verifies irqbalance configuration handling Should DO overwrite the banned CPU set on CRI-O restart | 
| [performance] Checking IRQBalance settings Verify irqbalance configuration handling Should not overwrite the banned CPU set on tuned restart | Verifies irqbalance configuration handling Should not overwrite the banned CPU set on tuned restart | 
| [performance] Checking IRQBalance settings Verify irqbalance configuration handling Should store empty cpu mask in the backup file | Verifies irqbalance configuration handling Should store empty cpu mask in the backup file | 
