...
| [dpdk] Rootless vhostnet Validate traffic on a rootless pod should receive packets on the tap and vlan subinterfaces | Verifies that a rootless pod can inject traffic to the kernel using a tap device | 
...
| [sriov] operator Custom SriovNetworkNodePolicy Configuration ExcludeTopology field is forwarded to the device plugin configuration | Verifies that ExcludeTopology field is correctly forwarded to the device plugin configuration | 
| [sriov] operator Custom SriovNetworkNodePolicy Configuration ExcludeTopology multiple values for the same resource should not be allowed | Verifies that multiple policies for the same resource can not have different values of ExcludeTopology | 
...
| Test Name | Description |
| -- | ----------- |
| [oc-serial] PTP configuration verifications Should check that all nodes are running at least one replica of linuxptp-daemon | Checks if the linuxptp-daemon is running on all the nodes. | 
| [oc-serial] PTP e2e tests PTP Interfaces discovery The interfaces supporting ptp can be discovered correctly | Checks if the interfaces supporting ptp are discovered correctly. | 
| [oc-serial] PTP e2e tests Running with event enabled Should check for ptp events | Checks for ptp events | 
...
| [oc-serial] PTP configuration verifications Should check that operator is deployed | Checks if the ptp operator is deployed. | 
| [oc-serial] PTP configuration verifications Should check whether PTP operator appropriate resource exists | Checks if the ptp operator CRDs exist on the cluster. | 
| [oc-serial] PTP configuration verifications Should check whether PTP operator needs to enable PTP events | Checks whether PTP operator needs to enable PTP events | 
| [oc-serial] PTP e2e tests PTP ClockSync Can provide a profile with higher priority | Checks that ptp operator can provide a profile with higher priority | 
| [oc-serial] PTP e2e tests PTP ClockSync Downstream slave can sync to BC master | Checks that downstream slave can sync to BC master | 
| [oc-serial] PTP e2e tests PTP ClockSync PTP daemon apply match rule based on nodeLabel | Checks that PTP daemon apply match rule based on nodeLabel | 
| [oc-serial] PTP e2e tests PTP ClockSync Slave can sync to master | Checks that Slave can sync to master | 
| [oc-serial] PTP e2e tests PTP Interfaces discovery Should retrieve the details of hardwares for the Ptp | Checks that PTP Interfaces discovery retrieves the details of hardwares for the Ptp | 
| [oc-serial] PTP e2e tests PTP Outage recovery The slave node network interface is taken down and up | XXXXXXXX | 
| [oc-serial] PTP e2e tests PTP Reboot discovery The slave node is rebooted and discovered and in sync | Checks that the slave node is rebooted and discovered and in sync | 
| [oc-serial] PTP e2e tests PTP metric is present on slave | Checks that the metrics related to ptp are produced by the slave. | 
| [oc-serial] PTP e2e tests PTP socket sharing between pods Negative - run pmc in a new unprivileged pod on the slave node Should not be able to use the uds | Verifies that ptp uds socket cannot be used by an unprivileged pod on the slave node | 
| [oc-serial] PTP e2e tests PTP socket sharing between pods Run pmc in a new pod on the slave node Should be able to sync using a uds | Verifies that ptp uds socket is shared between pods on the slave node | 
| [oc-serial] PTP e2e tests Running with fifo scheduling Should check whether using fifo scheduling | Checks whether using fifo scheduling | 
| [oc-serial] PTP e2e tests Running with reference plugin Should check whether plugin is loaded | XXXXXXXX | 
| [oc-serial] PTP e2e tests Running with reference plugin Should check whether test plugin executes | XXXXXXXX | 
| [oc-serial] PTP e2e tests prometheus Metrics reported by PTP pods Should all be reported by prometheus | Verifies that the PTP metrics are reported. | 
