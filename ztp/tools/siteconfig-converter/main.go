...
func main() {
	var (
		inputFile           = flag.String("f", "", "Path to the SiteConfig YAML file")
		outputDir           = flag.String("d", ".", "Output directory for converted ClusterInstance files")
		clusterTemplate     = flag.String("t", "open-cluster-management/ai-cluster-templates-v1", "Template reference for Cluster (format: namespace/name)")
		nodeTemplate        = flag.String("n", "open-cluster-management/ai-node-templates-v1", "Template reference for Nodes (format: namespace/name)")
	...
	if *inputFile == "" {
		fmt.Println("Usage: siteconfig-converter -f <siteconfig.yaml> [-d output_dir] [-t cluster_namespace/name] [-n node_namespace/name] [-m configmap1,configmap2,...] [-s manifest1,manifest2,...]")
		fmt.Println("\nExamples:")
		fmt.Println("  siteconfig-converter -f example-siteconfig.yaml -d ./output")
		fmt.Println("  siteconfig-converter -f example-siteconfig.yaml -d ./output -t open-cluster-management/ai-cluster-templates-v1 -n open-cluster-management/ai-node-templates-v1")
		fmt.Println("  siteconfig-converter -f example-siteconfig.yaml -m extra-manifests-cm1,extra-manifests-cm2")
		fmt.Println("  siteconfig-converter -f example-siteconfig.yaml -s manifest1,manifest2 -m extra-manifests-cm1")
		os.Exit(1)
	}
	...
	err = convertToClusterInstance(siteConfig, *outputDir, *clusterTemplate, *nodeTemplate, *extraManifestsRefs, *suppressedManifests)
	if err != nil {
