	...
	for _, cluster := range siteConfig.Spec.Clusters {
		...
		if cluster.ApiVIP != "" {
			warningsCollector.AddWarning("WARNING: apiVIP is removed in ClusterInstance. " +
				"Use apiVIPs instead. If you are doing a live cluster migration, you need to create a custom template for AgentClusterInstall or suppress it.")
		}

		if cluster.IngressVIP != "" {
			warningsCollector.AddWarning("WARNING: ingressVIP is removed in ClusterInstance. " +
				"Use ingressVIPs instead. If you are doing a live cluster migration, you need to create a custom template for AgentClusterInstall or suppress it.")
		}
