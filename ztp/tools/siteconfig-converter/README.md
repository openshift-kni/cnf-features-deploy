...
This process:
1. Extracts all the extra manifests using the `siteconfig-generator` binary
2. Creates a `kustomization-configMapGenerator-snippet.yaml` file which includes a kustomize `configMapGenerator` that will sync the extra manifests ConfigMap on the hub cluster.
