...
class PolicyGenWrapper(Logger):
    def __init__(self, paths: list, resourcename: str = 'siteconfigs'):
        try:
            ...
            cwd = '/tmp/ztp/ztp-policy-generator'

            # Initialize the templating variables
            args = {'input': paths[0], 'output': paths[1]}
            if resourcename == 'siteconfigs':
                args['namespace'] = 'cluster-config-generator'
                args['template'] = '../source-cluster-crs'
            else:
                args['namespace'] = 'acm-policy-generator'
                args['template'] = '../source-policy-crs'

            # Render policyGenerator.yaml template into cwd
            with open('oneliner.yaml.j2', 'r') as tf:
                t = tf.read()
            tm = Template(t)
            pgy = tm.render(args=args)

            # Write the oneliner config
