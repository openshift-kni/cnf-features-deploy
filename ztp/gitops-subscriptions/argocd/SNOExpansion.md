...
#### __Notes:__
1. `cpuset` should not be configured for the worker node. Workload partitioning for workers is added via management policies after the node installation is completed
...
EOF
```
Please note that the `ClusterGroupUpgrade` CR can be applied to the hub simultaneously with the worker node policies. In this case the remediation will start immediately after the worker node is approved by the cluster.