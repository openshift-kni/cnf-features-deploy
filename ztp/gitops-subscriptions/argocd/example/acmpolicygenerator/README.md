# ACM PolicyGenerator 
The ACM Policy Generator Custom Resource (CRs) is the official template mechanism used to generate ACM policies. It is replacing Policy Generator Templates or PGT.
ACM Policy Generator CRs are similar to PGT but there are few notable differences regarding its patching and placement strategies.
...
| PGT patch | ACM Generator patch |
|-----------|---------------------|
| Uses Strategic merge [link](https://github.com/kubernetes/community/blob/master/contributors/devel/sig-api-machinery/strategic-merge-patch.md)|PGT patches work by replacing variables with their values as defined by the patch|
|Relies only on patching, no embedded variable substitution|Overwrites values defined in patch|
|Does not support merging lists, only replace (missing support for schema)|Substitute variables defined in source CR with values defined in patch (for example $name)|
...
Using ACM Policy Generator templatess is similar to PGT as describe in chapter 3 of the following readme describing the overall ZTP process [link](https://github.com/openshift-kni/cnf-features-deploy/blob/master/ztp/gitops-subscriptions/argocd/README.md)

3. Create the ACM Generator Template CR for your site in your local clone of the git repository:
...
The pgt2acm tool supports creating such a patch that contains the full content of the CR, see below.

# Converting ACM Policy Generator templatess from exising Policy Gen Templates 
The pgt2acm supports converting Policy Gen Templates to ACM Policy Generator templatess◊. More details can be found at [link](https://github.com/test-network-function/pgt2acm/blob/main/README.md)
