...
func generateACMResourceDefinitions(t *testing.T) {
	InitiatePolicyGen (getTemplatePath(t), getSourcePolicyPath(t),getOutPath(t), true,false)
}

func generateCustomResourceDefinitions(t *testing.T) {
	InitiatePolicyGen (getTemplatePath(t), getSourcePolicyPath(t),getOutPath(t), true,true)
}
...
func CustomResourceDefinitionAssertions(t *testing.T) {
	assert.True(t,checkDirectoryExists(getOutPath(t), CUSTOM_RESOURCE_DIRECTORY))
	var customResourceDir = path.Join(getOutPath(t), CUSTOM_RESOURCE_DIRECTORY)
...
func getSourcePolicyWithSubstitutions(t *testing.T) map[string]interface{}{
	...
	for i := 0; i < len(policyTemp.SourceFiles); i++ {
		...
		if sFile.Name != "" && sFile.Name != utils.NotApplicable {
			fileData["metadata"].(map[string]interface{})["name"] = sFile.Name
		}
		if len(sFile.Labels) > 0 {
			fileData["metadata"].(map[string]interface{})["labels"] = sFile.Labels
		}
		if sFile.Spec != nil {
...
func getACMObjectDefinitionFromGeneratedYaml(generatedFilePath string,t *testing.T) map[string]interface{}{
	...
	return fileData[SPEC].(map[string]interface{})[POLICY_TEMPLATES].
	([]interface{})[0].(map[string]interface{})[OBJECT_DEFINITION].
	(map[string]interface{})[SPEC].
	(map[string]interface{})[OBJECT_TEMPLATES].
	([]interface{})[0].(map[string]interface{})[OBJECT_DEFINITION].
